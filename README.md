Wantors is a top-down shooter roguelite game made with Godot.

# Installation

1. Go to [Releases](https://github.com/Javier-AN/wantors-resist/releases), the top one is the latest.
2. Under Assets, download the specific zip for your OS.
3. Unzip it.
4. Run the executable to open the game.

# Licenses

- Texts, logos and the name Wantors: All rights reserved, no license granted. May be shown and used to showcase the game.
- Source code: [The Unlicense](https://unlicense.org).
- Sprites: [Attribution-NonCommercial-NoDerivatives 4.0 International](https://creativecommons.org/licenses/by-nc-nd/4.0/).
- Other assets: See credits.

# Credits

## Music

- [Jumpack Music Loops](https://dani-maccari.itch.io/jumppack-music-loops), by Dani Maccari in collaboration with Tallbeard Studios.

## SFX

- [Pixel Game Essentials SFX Pack](https://jdsherbert.itch.io/pixel-game-essentials-sfx-pack), by JDSherbert.
- [Pixel UI SFX Pack](https://jdsherbert.itch.io/pixel-ui-sfx-pack), by JDSherbert.

## Fonts

- [Dogica](https://www.dafont.com/dogica.font), by Roberto Mocci.
- [Pixel Operator](https://www.dafont.com/es/pixel-operator.font), by Jayvee Enaguas.

## Color palette

- [Neon Space](https://lospec.com/palette-list/neon-space), by Jimison3.
